...
const supportedDiagram = {
    kroki: {
        ...
        matchRegexp: /(\s*)(```) *(actdiag|blockdiag|bpmn|bytefield|c4plantuml|ditaa|erd|excalidraw|graphviz|mermaid|nomnoml|nwdiag|packetdiag|pikchr|p(?:lant)?uml|rackdiag|seqdiag|svgbob|umlet|vega(?:lite)?|wavedrom) *\n?([\s\S]+?)\s*(\2)(\n+|$)/g,
        //  curl -fsSL https://kroki.io/health | jq -r '.version | del(.kroki) | keys'
        diagTypes: [
            "actdiag",
            "blockdiag",
            "bpmn",
